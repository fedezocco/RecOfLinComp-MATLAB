...
function [RLC, T_discarded_hat, trainState_RLC] = rlc_pca(T_lin, k_lin, k, v_HiddenLayers)

if k < k_lin
    T = T_lin(:,1:k);
    T_discarded = T_lin(:,k+1:k_lin);
    RLC = feedforwardnet(v_HiddenLayers);
    [RLC, trainState_RLC] = train(RLC,T',T_discarded'); 
    T_discarded_hat = RLC(T')';
else
    disp('No need to recover the linear components with RLC, PCA is already enough')
    RLC = [];
    T_discarded_hat = [];
end